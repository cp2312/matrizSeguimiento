      ...
      {usuarios && (
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl overflow-hidden">
          <table className="w-full text-sm">
            ...
            <tbody>
              {usuarios.map((u) => (
                <tr key={u.id} className={`border-t border-slate-100 dark:border-slate-800 ${u.active ? '' : 'opacity-50'}`}>
                  <td className="px-4 py-3">
                    <p className="text-slate-800 dark:text-slate-100">{u.full_name}</p>
                    <p className="text-xs text-slate-500 dark:text-slate-400">{u.email}</p>
                  </td>
                  <td className="text-center text-slate-600 dark:text-slate-300">{u.initials}</td>
                  <td className="px-2">
                    {u.role === 'administrador' ? (
                      <span className="text-xs bg-slate-100 dark:bg-white/10 text-slate-700 dark:text-slate-200 px-2 py-1 rounded">
                        Administrador
                      </span>
                    ) : (
    ...
    >
      <form onSubmit={enviar} className="space-y-4">
        ...
        <Campo
          ...
          className={esEdicion ? 'bg-slate-50 text-slate-500 dark:bg-slate-800/60 dark:text-slate-400' : ''}
        />
        ...
        {esEdicion && (
          <label className="flex items-center gap-2.5 cursor-pointer">
            <input
              ...
              className="w-4 h-4"
            />
            <span className="text-sm text-slate-700 dark:text-slate-300">Cuenta activa</span>
          </label>
        )}
